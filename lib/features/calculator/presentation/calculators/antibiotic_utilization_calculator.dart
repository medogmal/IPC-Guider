...
class AntibioticUtilizationCalculator extends ConsumerStatefulWidget {
  const AntibioticUtilizationCalculator({super.key});

  @override
  ConsumerState<AntibioticUtilizationCalculator> createState() => _AntibioticUtilizationCalculatorState();
}

class _AntibioticUtilizationCalculatorState extends ConsumerState<AntibioticUtilizationCalculator> {
  ...
  final _knowledgePanelData = const KnowledgePanelData(
    definition: 'Antibiotic Utilization Percentage is a simplified antimicrobial use metric that measures the proportion of patient days with antibiotic exposure. It provides a quick, easy-to-understand snapshot of antibiotic use intensity across a unit or facility. This metric is particularly useful for communicating stewardship performance to non-technical stakeholders.',
    formula: '(Antibiotic Days × 100) / Total Patient Days',
    example: '500 days of antibiotic therapy over 10,000 patient days → (500 × 100) / 10,000 = 5% utilization',
    interpretation: 'Lower utilization percentages indicate less antibiotic exposure and better antimicrobial stewardship. High utilization may suggest overuse or inappropriate prescribing. This metric is essentially DOT divided by 10, providing the same information in percentage format. Benchmarks vary significantly by unit type, with ICUs typically having higher utilization than general wards.',
    whenUsed: 'Use this calculator for quick assessment of antibiotic use intensity and for communicating stewardship metrics to administrators, quality committees, and non-clinical stakeholders. Essential for dashboard reporting, trend monitoring, and setting reduction targets. Calculate monthly or quarterly for internal benchmarking and quality improvement initiatives.',
            ...
            child: Center(
              child: LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  return Math.tex(
                    r'\text{Utilization \%} = \frac{\text{Antibiotic Days} \times 100}{\text{Total Patient Days}}',
      ...
      child: Column(
        ...
        children: [
          ...
          TextFormField(
            ...
            decoration: InputDecoration(
              labelText: 'Antibiotic Days',
              hintText: 'Enter days with antibiotic use',
              suffixText: 'days',
              border: const OutlineInputBorder(),
            ...
            validator: (value) {
              if (value == null || value.isEmpty) {
                return 'Please enter antibiotic days';
              }
          ...
          TextFormField(
            ...
            validator: (value) {
              if (value == null || value.isEmpty) {
                return 'Please enter total patient days';
              }
              final num = int.tryParse(value);
              if (num == null || num < 1) {
  ...
  void _calculate() {
    ...
    Future.delayed(const Duration(milliseconds: 500), () {
      ...
      final utilization = (antibioticDays * 100) / patientDays;
      ...
      // Generate interpretation (Lower % = Better)
      String interpretation;
      if (utilization <= lowThreshold) {
        interpretation = 'Excellent Stewardship: Antibiotic utilization of ${utilization.toStringAsFixed(1)}% is low for $_selectedUnitType, indicating excellent antimicrobial stewardship. Antibiotic exposure is well-controlled. Continue current practices and monitor trends.';
      } else if (utilization <= moderateThreshold) {
        interpretation = 'Acceptable Use: Antibiotic utilization of ${utilization.toStringAsFixed(1)}% is within acceptable range for $_selectedUnitType. Antibiotic use is moderate. Review prescribing patterns to identify opportunities for further optimization.';
      } else if (utilization <= highThreshold) {
        interpretation = 'Elevated Use: Antibiotic utilization of ${utilization.toStringAsFixed(1)}% is elevated for $_selectedUnitType. This suggests potential overuse or prolonged therapy. Stewardship intervention is recommended to reduce unnecessary antibiotic exposure.';
      } else {
        interpretation = 'High Use - Action Required: Antibiotic utilization of ${utilization.toStringAsFixed(1)}% is significantly elevated for $_selectedUnitType. This indicates substantial antibiotic overuse. Immediate stewardship intervention is required to reduce exposure and prevent resistance.';
      }

      // Benchmark information
      final benchmark = 'Utilization Benchmarks for $_selectedUnitType: ≤${lowThreshold.toStringAsFixed(0)}% (excellent), ${lowThreshold.toStringAsFixed(0)}-${moderateThreshold.toStringAsFixed(0)}% (acceptable), ${moderateThreshold.toStringAsFixed(0)}-${highThreshold.toStringAsFixed(0)}% (elevated), >${highThreshold.toStringAsFixed(0)}% (high - action required)';

      // Action recommendations if utilization is elevated
      String? action;
      if (utilization > highThreshold) {
        action = 'Immediate Actions for High Utilization:\n'
        ...
        action = 'Recommended Actions to Reduce Utilization:\n'
  ...
  Map<String, double> _getUnitBenchmarks(String unitType) {
    switch (unitType) {
      case 'ICU':
        return {'low': 60.0, 'moderate': 80.0, 'high': 100.0};
      case 'Emergency Department':
        return {'low': 10.0, 'moderate': 20.0, 'high': 30.0};
      case 'Surgical Ward':
        return {'low': 40.0, 'moderate': 60.0, 'high': 80.0};
      case 'General Ward':
      default:
        return {'low': 30.0, 'moderate': 50.0, 'high': 70.0};
    }
  }
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      OutlinedButton.icon(
                        onPressed: () {
                          Clipboard.setData(ClipboardData(text: _utilization!.toStringAsFixed(1)));
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
  ...
  Future<void> _saveResult() async {
    try {
      ...
      final historyEntry = HistoryEntry.fromCalculator(
        calculatorName: 'Antibiotic Utilization Calculator',
        inputs: {
        ...
        result: 'Utilization: ${_utilization!.toStringAsFixed(1)} per $_selectedUnitType',
        notes: '',
        tags: ['antimicrobial-stewardship', 'antibiotic-utilization', 'surveillance'],
      );
  ...
  Future<void> _exportAsPDF() async {
    await UnifiedExportService.exportCalculatorAsPDF(
      context: context,
      toolName: 'Antibiotic Utilization % Calculator',
      formula: _knowledgePanelData.formula,
      inputs: {
        'Antibiotic Type': _selectedAntibioticType,
        'Unit Type': _selectedUnitType,
        'Antibiotic Days': _antibioticDaysController.text,
        'Total Patient Days': _patientDaysController.text,
      },
      results: {
        'Antibiotic Utilization': '${_utilization!.toStringAsFixed(1)}%',
      },
      benchmark: {
        'target': '≤50%',
        'unit': 'utilization',
        'source': 'WHO Antimicrobial Stewardship',
        'status': _utilization! > 50 ? 'Above Target' : 'Meets Target',
      },
